...
import data_manager
import push_results
from nlc_engine import compute_nlc_scores
...
def main():
    ...
    for universe_name, tickers in config.UNIVERSES.items():
        print(f"\n=== Universe: {universe_name} (NLC Engine) ===")
        ...
        for win in config.WINDOWS:
            min_bars = win + config.VOL_LONG_WIN + 5
        ...
        top_etfs    = [
            {"ticker": t, "nlc_score": float(s), "best_window": int(w)}
        ...
        for win, score_dict in window_results.items():
            ...
            windows_tab2[str(win)] = {
                "top_etfs":    [{"ticker": t, "nlc_score": float(s)} for t, s in sw[:config.TOP_N]],
    ...
    print(f"\n=== NLC Engine complete: {tab1_path.name}, {tab2_path.name} ===")
