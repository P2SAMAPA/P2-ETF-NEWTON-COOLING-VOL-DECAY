...
st.set_page_config(page_title="Newton Cooling Vol Decay Engine", layout="wide")
...
.uni-title   { font-size:1.4rem; font-weight:600; margin-top:1rem; margin-bottom:0.8rem;
               padding-left:0.5rem; border-left:5px solid #e76f51; }
.etf-card    { background:linear-gradient(135deg,#0d1b2a 0%,#e76f51 100%); color:white;
               border-radius:14px; padding:1rem; margin:0.4rem; text-align:center;
               box-shadow:0 4px 6px rgba(0,0,0,0.2); }
.win-card    { background:linear-gradient(135deg,#0d1b2a 0%,#c1440e 100%); color:white;
               border-radius:14px; padding:1rem; margin:0.4rem; text-align:center;
               box-shadow:0 4px 6px rgba(0,0,0,0.2); }
...
st.markdown('<div class="main-header">🌡️ Newton Cooling Vol Decay Engine</div>',
            unsafe_allow_html=True)
st.markdown(
    '<div class="sub-header">Newton (1701) law of cooling · '
    'Post-spike vol decay fitting · '
    'OLS log-linearised cooling rate k · '
    'Multi-window cross-sectional z-score</div>',
    unsafe_allow_html=True)

st.sidebar.markdown("## Newton Cooling Engine")
st.sidebar.markdown(f"**Next Trading Day:** `{next_trading_day()}`")
st.sidebar.markdown(f"**Windows:** {config.WINDOWS}")
st.sidebar.markdown(
    f"**Spike threshold:** {config.SPIKE_THRESH_SIGMA}x long-run vol | "
    f"**Short vol win:** {config.VOL_SHORT_WIN}d | "
    f"**Long vol win:** {config.VOL_LONG_WIN}d")
st.sidebar.markdown(
    f"**Weights:** Cooling rate {config.WEIGHT_COOLING_RATE:.0%} | "
    f"Overshoot {config.WEIGHT_OVERSHOOT:.0%} | "
    f"Ambient {config.WEIGHT_AMBIENT:.0%}")
...
tab1_path = find_latest(files, "nlc_engine_2")
tab2_path = find_latest(files, "nlc_engine_windows_")
...
# ══════════════════════════════════════════════════════════════════════════════
with tab1:
    st.header("🏆 Top ETFs — Newton Cooling Vol Decay Signal")

    with st.expander("Newton Cooling Law Methodology", expanded=True):
        st.markdown("""
**Newton's Law of Cooling** (1701): a body at temperature T cools toward
ambient temperature T_inf at a rate proportional to the temperature excess:
...
**Applied to volatility decay after spikes:**

```
sigma(t) = sigma_inf + (sigma_0 - sigma_inf) * exp(-k * t)
```

Where sigma_0 = vol at spike peak, sigma_inf = long-run ambient vol (63d),
k = cooling rate constant (fitted per episode via OLS).

**Log-linearised OLS fit:**
```
log(sigma(t) - sigma_inf) = log(sigma_0 - sigma_inf) - k * t
```
...
|-----------|-------|--------|
| Cooling rate k | High = fast decay | Positive (self-correcting market) |
| Overshoot | sigma_today vs sigma_predicted | Positive when vol above trajectory |
| Ambient sigma_inf | Long-run vol level | Positive when structurally calm |

**Distinct from HAR-RV (in suite):** HAR models vol as autoregressive lags.
Newton cooling models the *functional form* of post-spike decay — an
exponential trajectory fitted directly from physics.
        """)
...
<div class="etf-card">
  ...
  <div class="etf-score">NLC score = {etf['nlc_score']:.4f}</div>
        ...
        with st.expander(f"Full ranking — {universe_name}"):
            ...
            if full:
                ...
                for t, info in full.items():
                    ...
                    rows.append({"ETF": t, "NLC Score": score, "Best Window (d)": win})
                df = pd.DataFrame(rows).sort_values("NLC Score", ascending=False)
...
# ══════════════════════════════════════════════════════════════════════════════
with tab2:
    st.header("🔍 Explore NLC Rankings by Window")
    ...
    with st.expander("Window guidance", expanded=False):
        st.markdown("""
- **63d** — captures 2-3 spike episodes; reactive to recent vol regime
- **126d** — 6-month window; more spike episodes for averaging cooling rate k
- **252d** — 1-year window; stable k estimates; recommended primary signal
- **504d** — 2-year window; structural cooling dynamics; slow-moving
        """)

    st.markdown(f"### NLC Rankings at **{selected_win}d** window")
...
""", unsafe_allow_html=True)
        ...
        with st.expander(f"Full ranking — {label} @ {selected_win}d"):
            ...
            if rows:
                df = pd.DataFrame(rows, columns=["ETF", "NLC Score"])
